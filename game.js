...
function drawTelegraphs(){if(jan.skills.Q.state==="q2"){ctx.save();ctx.translate(state.player.x,state.player.y);ctx.rotate(state.player.facing);ctx.strokeStyle="#e4e8ee";ctx.globalAlpha=.22;ctx.strokeRect(0,-cfg.Q_WIDTH/2,cfg.Q_LENGTH,cfg.Q_WIDTH);ctx.restore()}for(const t of state.telegraph){ctx.save();ctx.translate(t.x,t.y);ctx.rotate(t.a);ctx.globalAlpha=.25;ctx.fillStyle="#e3e8ee";ctx.beginPath();ctx.moveTo(0,0);ctx.arc(0,0,cfg.W_RADIUS,-cfg.W_HALF_ANGLE,cfg.W_HALF_ANGLE);ctx.closePath();ctx.fill();ctx.restore()}if(state.rRing){const a=state.rRing.life/state.rRing.max;ctx.save();ctx.globalAlpha=a*.5;ctx.strokeStyle="#eef1f5";ctx.lineWidth=4;ctx.beginPath();ctx.arc(state.rRing.x,state.rRing.y,cfg.R_INNER_RADIUS,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.arc(state.rRing.x,state.rRing.y,cfg.R_OUTER_RADIUS,0,Math.PI*2);ctx.stroke();ctx.restore()}}
function drawEffects(dt){for(const p of state.particles){p.life-=dt;const alpha=Math.max(0,p.life/p.max);ctx.save();ctx.globalAlpha=alpha;if(p.type==="hit"){ctx.strokeStyle="#eef1f5";ctx.beginPath();ctx.arc(p.x,p.y,(1-alpha)*36+6,0,Math.PI*2);ctx.stroke()}else if(p.type==="dash"){ctx.strokeStyle="#e7ebf0";ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(p.tx,p.ty);ctx.stroke()}ctx.restore()}state.particles=state.particles.filter(p=>p.life>0);for(const f of state.floaters){f.life-=dt;const alpha=Math.max(0,f.life/f.max);ctx.save();ctx.globalAlpha=alpha;ctx.fillStyle="#eef1f5";ctx.font="bold 15px system-ui";ctx.textAlign="center";ctx.fillText(f.text,f.x,f.y-(1-alpha)*24);ctx.restore()}state.floaters=state.floaters.filter(f=>f.life>0)}
