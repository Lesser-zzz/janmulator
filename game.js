...
const state={
  ...
  dummies:[
    {id:1,x:485,y:285,r:31,hit:0,shake:0},
    {id:2,x:350,y:470,r:31,hit:0,shake:0},
    {id:3,x:885,y:250,r:31,hit:0,shake:0}
  ],
...
const stackRowEl=document.getElementById("stackRow");
...
function nearestDummyToPoint(point){
  let best=null,bestDistance=Infinity;
  for(const d of state.dummies){
    const dd=distance(point,d);
    if(dd<bestDistance){
      best=d;
      bestDistance=dd;
    }
...
function dummyAt(point){
  for(const d of state.dummies){
    if(distance(point,d)<=d.r+9)return d;
  }
  return null;
}

function nearestDummyInRange(){
  let best=null,bestDistance=Infinity;
  for(const d of state.dummies){
    const dd=distance(state.player,d);
    if(dd<=cfg.attackRange+d.r && dd<bestDistance){
      best=d;
      bestDistance=dd;
    }
  }
...
function attackRangeEnough(d){
  return d && distance(state.player,d)<=cfg.attackRange+d.r;
}

function faceAt(point){
  state.player.facing=Math.atan2(point.y-state.player.y,point.x-state.player.x);
}
...
function hitDummy(d,source){
  ...
  d.hit=.15;
  d.shake=.15;
  state.player.flash=.10;

  state.particles.push({x:d.x,y:d.y,life:.22,max:.22,type:"hit"});
  state.floaters.push({x:d.x,y:d.y-42,text:"+1",life:.6,max:.6});

  log(`<b>${source}</b> 적중 · 패시브 ${change.before} → ${change.after}`);
}

function beginBasicAttack(target){
  const p=state.player;
  if(!target || p.attackCooldown>0 || p.windup>0)return false;
  if(!attackRangeEnough(target))return false;

  p.target=target;
  p.windup=cfg.attackWindup;
  p.windupTarget=target;
  p.attackCooldown=cfg.attackInterval;
  faceAt(target);
  return true;
}

function finishBasicAttack(){
  const d=state.player.windupTarget;
  state.player.windupTarget=null;

  if(d && attackRangeEnough(d)){
    hitDummy(d,"평타");
  }
}
...
function issueAttack(point){
  ...
  state.player.attackMove=true;
  state.player.windup=0;
  state.player.windupTarget=null;
  modeEl.textContent=`공격 대상 DUMMY ${target.id}`;

  if(attackRangeEnough(target) && state.player.attackCooldown<=0){
...
function updatePlayer(dt){
  ...
  if(p.windup>0){
    p.windup-=dt;
    if(p.windup<=0)finishBasicAttack();
    return;
  }
...
function logCooldownChange(result){
  if(!result)return;
  const parts=[];

  for(const k of ["Q","W","E"]){
    const before=result.before[k]||0;
    const after=result.after[k]||0;
    if(before>0){
      parts.push(`${k} ${before.toFixed(2)}→${after.toFixed(2)}`);
    }
  }

  if(parts.length){
    log(`강화 쿨감 · ${parts.join(" / ")}`);
  }
}
...
function qHit(stage,enhancedCast){
  ...
  for(const d of state.dummies){
    ...
    if(
      forward>=0 &&
      forward<=cfg.Q_LENGTH &&
      Math.abs(side)<=cfg.Q_WIDTH/2+d.r
    ){
...
function wHit(enhancedCast){
  ...
  for(const d of state.dummies){
    ...
    if(r>cfg.W_RADIUS+d.r)continue;

    const delta=Math.abs(Math.atan2(
      Math.sin(Math.atan2(dy,dx)-a),
      Math.cos(Math.atan2(dy,dx)-a)
    ));

    if(delta<=cfg.W_HALF_ANGLE){
      hitDummy(d,`W${enhancedCast?"(강화)":""}`);
    }
  }
}
...
function castQ(){
  ...
  if(result.stage===1){
    ...
    if(result.enhanced){
      log("<b>강화 Q</b> · Q/W/E 전체 유효 쿨다운의 30% 차감");
...
function castE(){
  ...
  state.player.x=Math.max(
    state.player.r,
    Math.min(canvas.width-state.player.r,oldX+Math.cos(a)*cfg.E_DISTANCE)
  );
  state.player.y=Math.max(
    state.player.r,
    Math.min(canvas.height-state.player.r,oldY+Math.sin(a)*cfg.E_DISTANCE)
  );
  ...
  if(result.enhanced){
    log("<b>강화 E</b> · Q/W/E 전체 유효 쿨다운의 30% 차감");
    logCooldownChange(result.cooldownResult);
    if(result.eExtra){
      log(
        `<b>E 추가 쿨감</b> · E 전체 유효 쿨다운의 40% 차감 · `+
        `${result.eExtra.before.toFixed(2)}→${result.eExtra.after.toFixed(2)}`
      );
    }
  }else{
    log("<b>E</b> · 고정거리 돌진");
...
function updateSkills(dt){
  ...
  for(const t of state.telegraph){
    ...
    if(t.life<=0 && t.type==="W"){
      ...
      if(result.enhanced){
        log("<b>강화 W</b> · Q/W/E 전체 유효 쿨다운의 30% 차감");
        logCooldownChange(result.cooldownResult);
      }else{
        log("<b>W 판정 완료</b>");
      }
  ...
  state.telegraph=state.telegraph.filter(t=>t.life>0);

  if(state.rRing){
    state.rRing.life-=dt;
    if(state.rRing.life<=0)state.rRing=null;
  }
}

function updateDummyEffects(dt){
  for(const d of state.dummies){
...
function drawMoveMarker(){
  if(!state.player.destination)return;

  const p=state.player.destination;

  ctx.save();
  ctx.strokeStyle="#929baa";
  ctx.lineWidth=2;
...
function drawDummies(){
  for(const d of state.dummies){
    const jitter=d.shake>0?Math.sin(performance.now()/18+d.id)*3:0;

    ctx.save();
    ctx.translate(d.x+jitter,d.y);
    ...
    ctx.fillStyle=d.hit>0?"#f1f3f6":"#687383";
    ctx.fillRect(-24,-34,48,68);

    ctx.fillStyle="#38414d";
    ctx.fillRect(-14,-50,28,14);

    ctx.fillStyle="#11161c";
    ctx.font="bold 10px system-ui";
    ctx.textAlign="center";
    ctx.fillText("DUMMY "+d.id,0,60);
...
function drawPlayer(){
  ...
  ctx.strokeStyle="rgba(240,243,247,.08)";
  ctx.beginPath();
  ctx.arc(0,0,cfg.attackRange,0,Math.PI*2);
  ctx.stroke();
...
function drawTelegraphs(){
  ...
  for(const t of state.telegraph){
    ...
    ctx.beginPath();
    ctx.moveTo(0,0);
    ctx.arc(0,0,cfg.W_RADIUS,-cfg.W_HALF_ANGLE,cfg.W_HALF_ANGLE);
    ctx.closePath();
    ctx.fill();
    ctx.restore();
  }
  ...
  if(state.rRing){
    const a=state.rRing.life/state.rRing.max;
    ctx.save();
    ctx.globalAlpha=a*.5;
    ctx.strokeStyle="#eef1f5";
    ctx.lineWidth=4;
    ctx.beginPath();
    ctx.arc(state.rRing.x,state.rRing.y,cfg.R_INNER_RADIUS,0,Math.PI*2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(state.rRing.x,state.rRing.y,cfg.R_OUTER_RADIUS,0,Math.PI*2);
    ctx.stroke();
    ctx.restore();
  }
}
...
function drawEffects(dt){
  for(const p of state.particles){
    ...
    if(p.type==="hit"){
      ctx.strokeStyle="#eef1f5";
      ctx.beginPath();
      ctx.arc(p.x,p.y,(1-alpha)*36+6,0,Math.PI*2);
      ctx.stroke();
    }else if(p.type==="dash"){
  ...
  for(const f of state.floaters){
    ...
    ctx.textAlign="center";
    ctx.fillText(f.text,f.x,f.y-(1-alpha)*24);
    ctx.restore();
  }
...
function updateUI(){
  ...
  for(const k of ["Q","W","E","R"]){
    ...
    el.className="skill"+
      ...
      ((canEnhance||skill.state==="q2")?" ready":"");

    let name=k;
    if(k==="Q"&&skill.state==="q2")name="Q2";
    else if(canEnhance)name="강화 "+k;
    if(k==="W"&&skill.pending>0)name="W 대기";

    let time="READY";
    if(skill.pending>0)time=skill.pending.toFixed(1);
    else if(k==="Q"&&skill.state==="q2")time=skill.q2Timer.toFixed(1);
    else if(skill.cooldown>0)time=skill.cooldown.toFixed(1);

    el.innerHTML=`
      <div class="skill-key">${k}</div>
      ${(canEnhance&&!((k==="Q")&&skill.state==="q2"))?'<div class="skill-tag">ENHANCED</div>':""}
...
window.addEventListener("keydown",e=>{
  ...
  if(k==="a"){
    state.attackModifier=true;
    modeEl.textContent="A 모드 · 클릭하여 가장 가까운 더미 공격";
    e.preventDefault();
    return;
  }
...
canvas.addEventListener("mousemove",e=>{
  state.mouse=pointFromEvent(e);
});

for(const k of ["Q","W","E","R"]){
  document.getElementById("skill"+k).addEventListener("click",()=>castSkill(k));
}

document.getElementById("reset").addEventListener("click",reset);

log("훈련장 입장");
log("우클릭 = 이동 / A+좌클릭 = 커서 기준 가장 가까운 더미 공격");
